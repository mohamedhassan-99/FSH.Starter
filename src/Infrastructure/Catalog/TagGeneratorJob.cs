...
public class TagGeneratorJob : ITagGeneratorJob
{
    private readonly ILogger<TagGeneratorJob> _logger;
    ...
    public TagGeneratorJob(
        ILogger<TagGeneratorJob> logger,
        ISender mediator,
        IReadRepository<Tag> repository,
    ...
    public async Task CleanAsync(CancellationToken cancellationToken)
    {
        ...
        var items = await _repository.ListAsync(new RandomTagsSpec(), cancellationToken);

        _logger.LogInformation("Tags Random: {tagCount} ", items.Count.ToString());

        foreach (var item in items)
        {
            await _mediator.Send(new DeleteTagRequest(item.Id), cancellationToken);
        }

        _logger.LogInformation("All random tags deleted.");
    }
}

public class RandomTagsSpec : Specification<Tag>
{
    public RandomTagsSpec() =>
        Query.Where(b => !string.IsNullOrEmpty(b.Name) && b.Name.Contains("Tag Random"));
}